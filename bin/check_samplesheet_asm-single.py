#!/usr/bin/env python3
"""
Validate and transform the input samplesheet for the lrs_asm_single pipeline.

Checks that the samplesheet contains the required columns (sID, hifi_fastq,
HPO), validates file extensions, and ensures sample IDs are unique. Writes a
validated CSV to file_out.

Usage:
    check_samplesheet_asm-single.py <samplesheet.csv> <samplesheet.valid.csv>
...
class RowChecker:
    """
    Validate and transform each row of the single-sample samplesheet.
    ...
    def __init__(
        ...
        **kwargs
        ...
        Args:
            ...
            fastq_col (str): Column containing the HiFi FASTQ path.
    ...
    def _validate_fastq(self, row):
        """Assert that the FASTQ path is non-empty and has a valid extension."""
        if len(row[self._fastq_col]) <= 0:
            raise AssertionError("The HiFi FASTQ file path is required.")
        if not any(row[self._fastq_col].endswith(ext) for ext in self.VALID_FASTQ_FORMATS):
            raise AssertionError(
                f"Unrecognized FASTQ file extension: {row[self._fastq_col]}\n"
...
def check_samplesheet(file_in, file_out):
    ...
    """
    required_columns = {"sID", "hifi_fastq", "HPO"}
...
def parse_args(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description="Validate and transform a single-sample samplesheet.",
        epilog="Example: check_samplesheet_asm-single.py samplesheet.csv samplesheet.valid.csv",
    )
    parser.add_argument(
        "file_in",
        metavar="FILE_IN",
