...
========================================================================================
    Single-sample de novo genome assembly and variant calling pipeline for PacBio
    HiFi long-read sequencing data.

    Steps:
        00  Input validation and samplesheet parsing
        01  Single-sample de novo assembly (HiFiasm + samtools)
    ...
    Usage:
        nextflow run lrs_asm_single.nf -profile <profile> --csv_path <samplesheet.csv>
...
include { ASSEMBLY_HIFIASM;
          SAMTOOLS_HIFIASM                             } from './subworkflow/01_ASSEMBLE_HIFIASM.nf'
...
workflow {
    ...
    // ---------------------------------------------------------------------------
    // 01 Single-sample assembly
    // ---------------------------------------------------------------------------
    assembly_hifiasm_out = ASSEMBLY_HIFIASM(
        input_check_out.bams,
        hifiasm_threads
    )

    samtools_hifiasm_out = SAMTOOLS_HIFIASM(
        assembly_hifiasm_out.assembled_ctg
    )
    ...
    call_svs_pav_out = CALL_SVS_PAV(
        assembly_hifiasm_out.assembled_ctg,
        samtools_hifiasm_out.assembled_ctg_fai,
        file(reference_file),
    ...
    */
    ...
    publish:
    assembly_hifiasm_out.assembled_ctg                           >> 'assembly_hifiasm'
    samtools_hifiasm_out.assembled_ctg_fai                       >> 'samtools_hifiasm'
