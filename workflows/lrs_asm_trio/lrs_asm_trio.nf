...
output {
    'yak' {
        path { meta, pat_yak, mat_yak -> "${params.outputDir}${meta.id}/asm-trio/${params.runDate}/yak" }
        mode 'copy'
        overwrite false
    }
    'assembly_hifiasm_trio' {
        path { meta, out_asm_gfa, pat_yak, mat_yak, out_asm_hap1, out_asm_hap2 -> "${params.outputDir}${meta.id}/asm-trio/${params.runDate}/hifiasm_trio" }
        mode 'copy'
        overwrite false
    }
    'samtools_hifiasm_trio' {
        path { meta, out_utg_fa, out_utg_fa_fai -> "${params.outputDir}${meta.id}/asm-trio/${params.runDate}/hifiasm_trio" }
        mode 'copy'
        overwrite false
    }
    'call_svs_pav' {
        path { meta, pav_vcf_gz_rid, pav_vcf_gz_tbi_rid -> "${params.outputDir}${meta.id}/asm-trio/${params.runDate}/pav" }
        mode 'copy'
        overwrite false
    }
    'split_small_variants_pav' {
        path { meta, pav_snvs_vcf, pav_indels_vcf, pav_indels50bp_vcf -> "${params.outputDir}${meta.id}/asm-trio/${params.runDate}/pav" }
        mode 'copy'
        overwrite false
    }
    'index_small_variants_pav' {
        path { meta, pav_snvs_vcf_gz, pav_snvs_vcf_gz_tbi, pav_indels50bp_vcf_gz, pav_indels50bp_vcf_gz_tbi -> "${params.outputDir}${meta.id}/asm-trio/${params.runDate}/pav" }
        mode 'copy'
        overwrite false
    }
    'concat_small_variants_pav' {
        path { meta, out_smallvariants_pav, out_pav_vcf -> "${params.outputDir}${meta.id}/asm-trio/${params.runDate}/pav" }
        mode 'copy'
        overwrite false
    }
    'split_sv_pav' {
        path { meta, pav_SVins_vcf, pav_SVdel_vcf -> "${params.outputDir}${meta.id}/asm-trio/${params.runDate}/pav" }
        mode 'copy'
        overwrite false
    }
    'index_sv_pav' {
        path { meta, pav_SVins_vcf_gz, pav_SVdel_vcf_gz, pav_svins_vcf_gz_tbi, pav_svdel_vcf_gz_tbi -> "${params.outputDir}${meta.id}/asm-trio/${params.runDate}/pav" }
        mode 'copy'
        overwrite false
    }
    ...
    'concat_sv_pav' {
        path { meta, out_concat_sv_vcf -> "${params.outputDir}${meta.id}/asm-trio/${params.runDate}/pav" }
        mode 'copy'
        overwrite false
    }
    ...
    'prioritize_by_svanna' {
        path { meta, out_pav_svanna_csv -> "${params.outputDir}${meta.id}/asm-trio/${params.runDate}/svanna" }
        mode 'copy'
        overwrite false
    }
    'rscript_process_svanna_result' {
        path { meta, out_csv -> "${params.outputDir}${meta.id}/asm-trio/${params.runDate}/svanna" }
        mode 'copy'
        overwrite false
    }
}