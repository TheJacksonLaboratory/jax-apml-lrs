...
**NOTE:** JAX internal users please use the `jax-internal` branch, which is configured for use with Sumner2. See the `jax-internal` branch README for Sumner2-specific setup and submission instructions.
...
>All containers are pulled automatically from public registries at runtime. No local image builds are required. On first run, i
mages are downloaded and cached locally; subsequent runs will use cached images and skip the download step.

>**Note for HPC users:** Configure `apptainer.cacheDir` in your profile to a location with sufficient storage (containers can be several GB each).
...
| SvAnna | 1.0.4 | `svanna_v1.0.4.sif` (downloaded from `download_refs.sh`) | [github.com/TheJacksonLaboratory/SvAnna](https://github.com/TheJacksonLaboratory/SvAnna) |
