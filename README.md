...
- [Nextflow](https://www.nextflow.io/) version 24.10.5 or higher (bundled bootstrap script included, see [Quick Start](#quick-start))
  ...
  workflows/
    lrs_read/
      ...
      subworkflow/
        ...
        06_GENERATE_BAF_PLOT.nf
        07_VARIANT_SUMMARY_METRICS.nf
        08_PRIORITIZE_SV_SVANNA.nf
      modules/
...
>SvAnna requires local image build before running (all other containers are pulled automatically from public registries at runtime)

```bash  
#docker
docker build -t lrs-svanna:1.0.5 docker/svanna/

#Singularity/Apptainer 
singularity build lrs-svanna_1.0.5.sif docker-daemon://lrs-svanna:1.0.5
```
...
>This can be done with `download_refs.sh`, which uses the samtools container to generate the FASTA index using Docker (pass `--container-runtime singularity` to use Singularity instead)
```bash
# download files to `refs/` in repo root
./download_refs.sh
...
>The following small reference files are committed directly to the repository under `refs/` and require no download:
...
./run.sh -w <workflow> -p <profile> \
    ...
    --outputDir /path/to/output/ \
    --refs_path refs/ \ # not required for lrs_pbmerge
    --resume
...
| 06 | B-allele frequency and CN plots | R (Bioconductor) | |
| 07 | Variant summary metrics | bcftools stats, SURVIVOR | |
| 08 | Phenotype-driven SV prioritization | SvAnna, R | |
...
└── <sampleID>/
    └── <referenceID>/
        ...
        ├── b_allele/           BAF and CN PDF plots
        ├── svanna/             SvAnna prioritized SVs
        └── methbat/            methylation profiles (in development)
```
>NOTE: **Emedgene outputs:** Several processes produce `*_Emedgene.vcf` files (DeepVariant, PBSV). These are reformatted copies for ingestion into the [Emedgene](https://www.emedgene.com/) clinical interpretation platform. If not using Emedgene, use primary VCF outputs.
...
| samtools | 1.19 | `quay.io/biocontainers/samtools:1.19--h50ea8bc_1` | [github.com/samtools](https://github.com/samtools) |
...
| pbsv | 2.9.0 | `quay.io/pacbio/pbsv:2.9.0_build2` | [github.com/PacificBiosciences/pbsv](https://github.com/PacificBiosciences/pbsv)
...
| Delly | 1.2.6 | `quay.io/biocontainers/delly:1.2.6--hb7e2ac5_1` | [github.com/dellytools/delly](https://github.com/dellytools/delly) |
| HiFiCNV | 1.0.0 | `quay.io/pacbio/hificnv:1.0.0_build1` | [github.com/PacificBiosciences/HiFiCNV](https://github.com/PacificBiosciences/HiFiCNV) |
| TRGT | 1.0.0 | `quay.io/pacbio/trgt:1.0.0_build1` | [github.com/PacificBiosciences/trgt](https://github.com/PacificBiosciences/trgt) |
| GATK4 | 4.2.6.1 | `quay.io/biocontainers/gatk4:4.2.6.1--py36hdfd78af_1` | [github.com/broadinstitute/gatk](https://github.com/broadinstitute/gatk) |
...
| pb-CpG-tools | 2.3.2 | `quay.io/pacbio/pb-cpg-tools:2.3.2` | [github.com/PacificBiosciences/pb-CpG-tools](https://github.com/PacificBiosciences/pb-CpG-tools) |
...
| SVAFotate | 0.2.0 | `quay.io/biocontainers/svafotate:0.2.0--pyhdfd78af_0` | [github.com/fakedrtom/SVAFotate](https://github.com/fakedrtom/SVAFotate) |
| SvAnna | 1.0.5 | `lrs-svanna:1.0.5` (build from `docker/svanna/Dockerfile`) | [github.com/TheJacksonLaboratory/SvAnna](https://github.com/TheJacksonLaboratory/SvAnna) |
| SURVIVOR | 1.0.6.2 | `mgibio/survivor-cwl:1.0.6.2` | [github.com/fritzsedlazeck/SURVIVOR](https://github.com/fritzsedlazeck/SURVIVOR) |
| Bioconductor | RELEASE_3_18 | `quay.io/bioconductor/bioconductor_docker:RELEASE_3_18` | [bioconductor.org/](https://www.bioconductor.org/) |
| R dplyr | 1.1.4 | `quay.io/biocontainers/r-dplyr:1.1.4--r43hf9e5401_0` | [cran.r-project.org/web/packages/dplyr/vignettes/dplyr.html](https://cran.r-project.org/web/packages/dplyr/vignettes/dplyr.html) |
| pandas | 1.5.3 | `quay.io/biocontainers/pandas:1.5.3` | [github.com/pandas-dev/pandas](https://github.com/pandas-dev/pandas)|
...
| 04 | Phenotype-driven SV prioritization | SvAnna, R (dplyr) |
...
| Tool | Version | Container | Reference |
|---|---|---|---|
| Yak | 0.1 | `quay.io/biocontainers/yak:0.1--h5bf99c6_3` | [github.com/lh3/yak](https://github.com/lh3/yak) |
| HiFiasm | 0.20.0 | `dnalinux/hifiasm:0.20.0` | [github.com/chhylp123/hifiasm](https://github.com/chhylp123/hifiasm) |
| samtools | 1.19 | `quay.io/biocontainers/samtools:1.19--h50ea8bc_1` | [github.com/samtools](https://github.com/samtools) |
| PAV | 2.4.2 | `becklab/pav:2.4.2` | [github.com/EichlerLab/pav](https://github.com/EichlerLab/pav) |
| bcftools | 1.19 | `quay.io/biocontainers/bcftools:1.19--h8b25389_1` | [github.com/samtools/bcftools](https://github.com/samtools/bcftools) |
| tabix | 1.11 | `quay.io/biocontainers/tabix:1.11--hdfd78af_0` | [github.com/tabixio/tabix](https://github.com/tabixio/tabix)|
| SvAnna | 1.0.5 | `lrs-svanna:1.0.5` (build from `docker/svanna/Dockerfile`) | [github.com/TheJacksonLaboratory/SvAnna](https://github.com/TheJacksonLaboratory/SvAnna) |
| R dplyr | 1.1.4 | `quay.io/biocontainers/r-dplyr:1.1.4--r43hf9e5401_0` | [cran.r-project.org/web/packages/dplyr/vignettes/dplyr.html](https://cran.r-project.org/web/packages/dplyr/vignettes/dplyr.html) |
...
| 04 | Phenotype-driven SV prioritization | SvAnna, R |
...
└── <sampleID>/
    ...
    ├── hifiasm_trio/     assembled contigs and haplotype FASTAs
    ├── pav/              PAV VCFs — full, small variants, and SVs (reformatted for Emedgene ingestion)
    └── svanna/           SvAnna prioritized SVs and filtered candidates
```
